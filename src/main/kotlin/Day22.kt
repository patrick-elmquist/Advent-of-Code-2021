...
import Instruction.Off
import Instruction.On
import util.Input
import util.Point3D
import util.day
...
fun main() {
    day(n = 22) {
        testInput expect 39
        testInputLarge expect 590784
        part1(expected = 589411) { input ->
            val instructions = input.parse()

            val valid = -50..50
            val litCubes = mutableSetOf<Point3D>()
            instructions
                .filter {
                    val c = it.cuboid
                    c.x.first in valid && c.x.last in valid &&
                            c.y.first in valid && c.y.last in valid &&
                            c.z.first in valid && c.z.last in valid
                }
                .forEach { instruction ->
                    val c = instruction.cuboid
                    c.x.forEach { x ->
                        c.y.forEach { y ->
                            c.z.forEach { z ->
                                when (instruction) {
                                    is On -> litCubes.add(Point3D(x, y, z))
                                    is Off -> litCubes.remove(Point3D(x, y, z))
                                }
                            }
                        }
                }
        ...
        testInputHuge expect 2758514936282235L
        part2 { input ->
            val valid = -50..50
            val instructions = input.parse().map { it.cuboid }

            val cuboids = mutableListOf<Cuboid>()
            instructions.forEach { cuboid ->
                cuboids += cuboids
                    .filter { cuboid.intersect(it) }
                    .map { cuboid.intersection(it) }

                if (cuboid.sign == 1) cuboids += cuboid
            }

            cuboids.size.log()
            cuboids.sumOf { it.volume.toLong() * it.sign.toLong() }.log()
        }
    }
}

private fun Input.parse() = lines.map { Instruction(it) }
...
private data class Cuboid(
    ...
    fun intersection(other: Cuboid): Cuboid {
        ...
        return Cuboid(minX..maxX, minY..maxY, minZ..maxZ, sign)
    }
    ...
    companion object {
        operator fun invoke(line: String): Instruction =
            regex.match(line) { (state, x1, x2, y1, y2, z1, z2) ->
                val cuboid = Cuboid(x1, x2, y1, y2, z1, z2, state)
                if (state == "on") On(cuboid) else Off(cuboid)
            }
    }
}
